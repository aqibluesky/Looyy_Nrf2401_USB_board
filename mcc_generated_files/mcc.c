...
{
    OSCILLATOR_Initialize();    //频率初始化
    PIN_MANAGER_Initialize();   //引脚初始化
    //ADC_Initialize();           //ADC初始化
    //DAC1_Initialize();          //DAC初始化
    EUSART_Initialize();        //串口初始化 
  
    //TMR2_Initialize();          //定时器2初始化 产生10Hz方波    
    //TMR1_Initialize();          //定时器1初始化 
    //TMR0_Initialize();          //定时器0初始化
}
...
void OSCILLATOR_Initialize(void) {
    ...
    OSCSTAT = 0x00;  //频率状态寄存器
    // TUN 0x0; 
    OSCTUNE = 0x00;  //频率调节寄存器
