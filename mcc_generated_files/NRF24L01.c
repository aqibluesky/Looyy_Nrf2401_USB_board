...
/*主机模式（终端模式）*/
const unsigned char RX_ADDRESS[RX_ADR_WIDTH]={0x54,0x45,0xA0,0xA0,0xA0}; //数据盒子地址
const unsigned char TX_ADDRESS[TX_ADR_WIDTH]={0x45,0x54,0x0A,0x0A,0x0A}; //小模块接受地址
/*从机模式（小模块）*/
...
void RX_Mode(void)
{
	...
	NRF24L01_Write_Reg(NRF24L01_WRITE_REG+EN_AA,0x00);                                          //不是使能自动应答功能（广播模式）
	...
	NRF24L01_Write_Reg(NRF24L01_WRITE_REG+CONFIG, 0x1f);                                        //配置基本工作模式的参数;PWR_UP,EN_CRC,16BIT_CRC,接收模式
...
void TX_Mode(void)
{
	...
	NRF24L01_Write_Reg(NRF24L01_WRITE_REG+CONFIG,0x1e);                                         //配置基本工作模式的参数;PWR_UP,EN_CRC,16BIT_CRC,接收模式,开启所有中断
